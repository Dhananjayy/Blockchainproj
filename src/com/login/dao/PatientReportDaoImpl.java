...
public class PatientReportDaoImpl implements PatientReportIntf{
      ...
      public void addReport(PatientReport patientReport) {
		...
		try {
					...
					PreparedStatement ps = conn
							.prepareStatement("insert into patient_report(patient_id,report_name,report_date,report) values (?,?,?,?)");
	...
	public void deleteReport(int reportId) {
		try {
			conn = MyConnection.getConnectionObj();
			PreparedStatement ps = conn
					.prepareStatement("delete  from patient_report where id=? ");
	...
	public List<PatientReport> getAllReportByPatientId(int patientId) {
		try {
			conn = MyConnection.getConnectionObj();
			PreparedStatement ps = conn
					.prepareStatement("select * from patient_report where patient_id = ?");
			ps.setInt(1, patientId);
	...
	public PatientReport getReportById(int reportId) {
		try {
			conn = MyConnection.getConnectionObj();
			PreparedStatement ps = conn
					.prepareStatement("select * from patient_report where id = ?");
			ps.setInt(1, reportId);
