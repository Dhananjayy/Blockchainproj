...
public class PatientHistoryDaoImpl implements PatientHistoryIntf{
	...
	public void savePatientHistory(PatientHistory patientHistory) {
		...
		try {
			...
			PreparedStatement ps = conn
					.prepareStatement("insert into patient_history(patient_id,symptoms,prescription,prescribed_by_doctor,prescribed_by_name,prescribed_date) values (?,?,?,?,?,?)");
	...
	public List<PatientHistory> getAllbyPatientId(int patientId) {
		try {
			conn = MyConnection.getConnectionObj();
			PreparedStatement ps = conn
					.prepareStatement("select * from patient_history where patient_id =? ");
			...
			while (rs.next()) {
				...
				patientHistory.setId(rs.getInt("id"));
