...
public class PatientMasterDaoImpl implements PatientMasterInterface {
	...
	public boolean save(PatientMaster PatientMaster) {
		...
		try {
			...
			PreparedStatement ps = conn
					.prepareStatement("insert into patient_master(FIRST_NAME,LAST_NAME,CITY,STATE,PINCODE,CONTACT_NO,EMAIL,AGE,WEIGHT,HEIGHT,BLOOD_GROUP,USER_ID) values (?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	public boolean updatePatientMaster(PatientMaster PatientMaster) {
		try {
			conn = MyConnection.getConnectionObj();
			PreparedStatement ps = conn
					.prepareStatement("update patient_master set FIRST_NAME=?,LAST_NAME=?,CITY=?,STATE=?,PINCODE=?,CONTACT_NO=?,EMAIL=?,AGE=?,WEIGHT=?,HEIGHT=?,BLOOD_GROUP=? where  ID=? ");
	...
	public List<PatientMaster> PatientMasterList() {
		try {
			conn = MyConnection.getConnectionObj();
			PreparedStatement ps = conn
					.prepareStatement("select * from patient_master order by ID desc");
	...
	public boolean deletePatientMaster(PatientMaster PatientMaster) {
		try {
			conn = MyConnection.getConnectionObj();
			PreparedStatement ps = conn
					.prepareStatement("delete  from patient_master where id=? ");
	...
	public PatientMaster getElementById(PatientMaster PatientMaster) {
		try {
			conn = MyConnection.getConnectionObj();
			PreparedStatement ps = conn
					.prepareStatement("select * from patient_master where id= ?");
			ps.setInt(1, PatientMaster.getId());
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				PatientMaster.setId(rs.getInt("ID"));
	...
	public PatientMaster getElementByUserId(int userId) {
		try {
			conn = MyConnection.getConnectionObj();
			PreparedStatement ps = conn
					.prepareStatement("select * from patient_master where USER_ID = ?");
			ps.setInt(1, userId);
			PatientMaster patientMaster = new PatientMaster();
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
