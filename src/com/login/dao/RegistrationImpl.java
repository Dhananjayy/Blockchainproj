...
public class RegistrationImpl implements RegistrationInterface{
	...
	public boolean signup(Login l) {
		...
		try
		{
			...
			PreparedStatement pstm= conn.prepareStatement("insert into login_info(USERNAME,EMAIL_ID,STATE,CITY,PINCODE,PASSWORD,ADDRESS,role,status)values(?,?,?,?,?,?,?,?,?)");
	...
	public Login login(Login userLogin) {
		...
		Connection conn=null;
			...
			try
			{
				...
				conn=MyConnection.getConnectionObj();
				PreparedStatement ps=conn.prepareStatement("select * from login_info where EMAIL_ID=? and PASSWORD=? and role=?");
