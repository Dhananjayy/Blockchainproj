...
public class ImageUploadImpl implements ImageUploadInterface {
	...
	public long savepath(ImageUplaod imageUplaodObj,String mappingArray[]) {
		...
		try {
			long file_id=0;
	    PreparedStatement pstm=conn.prepareStatement("insert into image_post_table (PATH,USERNAME,fileSize,upload_date) values(?,?,?,now())");
		...
		PreparedStatement pstm1=conn.prepareStatement("select MAX(image_post_id) as image_post_id from image_post_table");
	...
	public boolean saveLogTable(ImageUplaod imageUplaodObj) {
	    ...
	    PreparedStatement pstm=conn.prepareStatement("insert into log_table (USERNAME,DATE,TIME,ACTION,FILE_NAME,IP_ADDRESS) values(?,?,?,?,?,?)");
