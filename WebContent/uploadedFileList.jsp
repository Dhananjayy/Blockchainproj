...
<link rel="Stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.4.0/css/font-awesome.min.css" />
        		...
        		$("#formwise").show();
        		 ...
        		 var button = document.getElementById("countButton");
        	      ...
        	      button.onclick = function(){
        	    	  ...
        	    	  $.ajax({
          		   		type:"POST",	
          		   		url:"UpdateStatusServlet?action=UpdateBlockUserStatus&status="+status,
          		   		data:{userInputViewKey:code},
          		   		success:function(abc){
    ...
    function GetFile(id)
    {
    	...
    	if(secretKey==mailedKey){
    		...
    		$("#SecretKeyDiv").hide();
    		//location.reload();
    		location.href="DownloadFile?id="+id+"&knum="+secretKey+"&name="+fileName;
    	}else{
    		  alert("wrong key");
...
<body background="http://wallpaperscraft.com/image/london_ride_river_house_dock_9656_3840x2400.jpg?orig=3">
	

    <div class="siteWrapper">
             <header class="header" style="background:-webkit-linear-gradient(left, #C9AA54 , #6D8475 ,#A00816);min-height:70px;"> 
                  ...
                  <div class="user col-sm-4 text-right">
                      <label>Welcome:: </label>
                      <a href="LogoutServletMaster" class="sign-out"> Sign Out <i class="fa fa-sign-out"></i></a>
                  </div>
              </header>
	<nav class="menu nav">
    <div class="container">
    ...
    <!--     <ul>
                ...
                <div class="panel-body" id="viewKeyDiv">
                    <div class="table-responsive col-sm-8">
                       ...
                       <div  class="col-sm-4" id="" >
                        <input type="hidden" id="viewKey" value="<%=(String)session.getAttribute("viewKey")%>"/>
                         <label>Enter Code :</label>  <br>
                        <input type="text" id="userInputViewKey" class="form-control" >
                        <input type="button" class="btn btn-primary" value="submit"  id="countButton" onclick="checkCode('<%=(String)session.getAttribute("viewKey")%>')">
					...
					for(int i=0;i<uploadedList.size();i++)
					{
						...
						%>
                            ...
                            <tr>
                                ...
                                <td><a onclick="getKeyByMail('<%=imageUpload.getPATH() %>','<%=imageUpload.getFile_id() %>')" class="export"><i class="fa fa-download"></i></a></td>
                        ...
                        <input type="hidden" id="mailedKey" value="<%=(String)session.getAttribute("key")%>"/>
                         <label>Enter Code :</label>  <br>
                        <input type="text" id="secretKey" class="form-control" >
                        <input type="button" class="btn btn-primary" value="GetFile"  id="countButton_1" onclick="GetFile('<%=(String)session.getAttribute("id")%>')">
                        <input type="hidden" id="fileName" value="<%=session.getAttribute("fileName") %>"></input>
                         <input type="hidden" id="displayCount_1" value="0"/>
                        </div>
                        <%} %>
                    </div>
                </div>
       ...
       {
    	%>   
    	<input type="hidden" id="userInput" value="<%=session.getAttribute("userInput")%>" class="form-control" >
     <%   }
