              ...
              <tbody> 
                 <c:forEach var="patientMaster" items="${patientMaterList}">
		               <tr>
		                 ...
		                 <td>${patientMaster.first_name} ${patientMaster.last_name}</td>
		                 <td>${patientMaster.age}</td>
		                 <td>${patientMaster.contact_no}</td>
		                 <td>${patientMaster.city}</td>
		                 <td>${patientMaster.state}</td>
		                 <td>
		                     <a href="PatientMasterServlet?action=Delete&id=${patientMaster.id}">Delete</a>
		                     <c:if test="${user.role=='Doctor'}">
			                 	<%-- <a href="PatientMasterServlet?action=Edit&id=${patientMaster.id}">Edit</a> --%>
			                 	<a href="PatientMasterServlet?action=View&id=${patientMaster.id}">View</a>
			                 </c:if>
		                 </td>
		                 <c:if test="${user.role=='Doctor'}">
                    		<td><a href="PatientHistoryAndReportsServlet?action=viewReports&patientId=${patientMaster.id}" >view</a></td>
                  		</c:if>
		               </tr>
